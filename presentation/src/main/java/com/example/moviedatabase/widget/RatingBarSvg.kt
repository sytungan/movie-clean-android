...
//            .setColorFilter(backgroundTintColor, PorterDuff.Mode.SRC_ATOP);
//        drawable.findDrawableByLayerId(android.R.id.progress)
//            .setColorFilter(progressTintColor, PorterDuff.Mode.SRC_ATOP);
    ...
    private fun tileify(drawable: Drawable, clip: Boolean): Drawable {
        if (drawable is DrawableWrapper) {
            ...
            return if (clip) ClipDrawable(shapeDrawable, Gravity.START, ClipDrawable.HORIZONTAL)
            else shapeDrawable
        } else {
